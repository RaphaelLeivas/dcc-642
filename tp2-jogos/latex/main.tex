...
O número de nós expandidos com a poda é comparado com o Minimax sem pode, de modo a verificar experimentalmente 
...
incrementando a profundidade máxima e usar o melhor lance 
encontrado entre todos as buscas, como mostra o Algoritmo \ref{alg:ids}.
...
\For{$d = 1$ to $D$}
    \State $move \gets \textbf{Minimax}(s, d, -\infty, +\infty, \textbf{true}, p)$
    \If{$move \neq \textbf{null}$}
...
Figura \ref{fig:histograma-minimax-nodes} número de nós expandidos
...
vez usando o minimax com a pode alta-beta vs o minimax, obtemos os 
...
Para o tempo limite de 1000 ms, o Iteractive Deepening consegue atingir a profundidade 
6. Com 2000 ms, atinge a profundidade 7. As vezes estourava o limite e usava o lança 
aleatório de fallback ao tentar atingir a pronfundidade 7 com 2000 ms, 
de modo que a performance com esse limite foi menor que a com 1000 ms. A Tabela \ref{tab:ids-alphabeta} sumariza os resultados obtidos 
nesse experimento.

\begin{table}[htbp] 
    \centering 
    \caption{Resultados finais do experimento Iteractice Deepning vs Alfa-beta.} 
    \label{tab:ids-alphabeta} 

    \begin{tabular}{c|c|c|c} 
        Limite (ms) & Vitórias (\%) & Tempo (ms) & Nós \\
		\hline 
        1000 & 100 & $1.7 \pm 2.7$ & $31 \pm 7.96$\\
        2000 & 0 & $6.4 \pm 3.8$ & $125.6 \pm 52.2$\\
    \end{tabular}
\end{table}
...
e ameaçar colcoar 3 bolas em sequência com os extremos abertos, uma ameaça 
dupla que vence o jogo.

\begin{figure}[htb]
	\centering 
    \caption{Jogada inicial da IA em todas as partidas.}
	\includegraphics[width=\columnwidth]{images/jogada-inicial-ia.png}
...
sintse do que funcionou melhor e limitações
